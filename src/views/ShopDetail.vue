<template>
  <div class="page">
    ...
    <!-- 固定搜索栏：背景、搜索框、爱心随下拉渐显 -->
    <div
        class="floating-header"
        :style="{ opacity: headerOpacity }"
    >
      <div class="header-content">
        <!-- 中间搜索组件（从右向左滑入，完全显示后才可交互） -->
        <div
            class="search-group"
            :style="{
...
const currentCategoryId = ref(1) // 默认选中第一个
...
const selectCategory = (id) => {
  ...
  if (tabBarFixed.value) {
    ...
    // 全局滚动模式，使用 scrollIntoView，依赖 .scroll-area 的 scroll-padding-top
...
const handleScroll = () => {
  ...
  // 1. 如果当前还没固定，实时记录条形容器在文档流中的位置
  if (!tabBarFixed.value) {
    tabBarInitialTop.value = tabBarRef.value.offsetTop
  }

  // 2. 判断是否该固定：滚动距离 >= 条形容器初始位置 - 搜索栏高度(76px)
